    ...
    _tap(e, move) {
      ...
      if (this.inScrollView === true || this.inAli) {
        let chartdom = uni
          ...
          .boundingClientRect((data) => {
            ...
            if (move) {
              ...
              this.emitMsg({ name: 'getIndex', params: { type: 'getIndex', event: { x: e.detail.x - data.left, y: e.detail.y - data.top }, currentIndex, legendIndex, id: cid, opts: cfu.instance[cid].opts } })
            }
          })
          .exec()
      }
        ...
        if (move) {
          ...
          this.emitMsg({ name: 'getIndex', params: { type: 'getIndex', event: { x: e.detail.x, y: e.detail.y - e.currentTarget.offsetTop }, currentIndex, legendIndex, id: cid, opts: cfu.instance[cid].opts } })
        }
      }
    },
    _touchStart(e) {
