...
function hexToRgb(hexValue, opc) {
  ...
  let hex = hexValue.replace(rgx, (m, r, g, b) => {
    return r + r + g + g + b + b
  })
...
function getSeriesDataItem(series, index, group) {
  let data = []
  let newSeries = []
  let indexIsArr = index.constructor.toString().includes('Array')
  if (indexIsArr) {
...
function findCurrentIndex(currentPoints, calPoints, opts, config) {
  ...
  if (calPoints && calPoints.length > 0) {
    ...
    if (isInExactChartArea(currentPoints, opts, config)) {
      if (!opts.categories) {
        let timePoints = Array.from({ length: calPoints.length })
        for (let i = 0; i < calPoints.length; i++) {
          timePoints[i] = Array.from({ length: calPoints[i].length })
          for (let j = 0; j < calPoints[i].length; j++) {
            timePoints[i][j] = (Math.abs(calPoints[i][j].x - currentPoints.x))
          }
        };
        let pointValue = Array.from({ length: timePoints.length })
        let pointIndex = Array.from({ length: timePoints.length })
...
function calCategoriesData(categories, opts, config, eachSpacing, context) {
  ...
  let categoriesTextLenth = categories.map((item, index) => {
...
function calXAxisData(series, opts, config, context) {
  ...
  let xAxisScaleValues = result.ranges.map((item) => {
    // 如果刻度值是浮点数,则保留两位小数
    item = util.toFixed(item, 2)
    // 若有自定义格式则调用自定义的格式化函数
...
function getRadarDataPoints(angleList, center, radius, series, opts) {
  let process = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1
...
function getCandleDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
  let process = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : 1
  let points = []
...
function drawCandleDataPoints(series, seriesMA, opts, config, context) {
  ...
  series.forEach((eachSeries, seriesIndex) => {
    ...
    maxRange = ranges.shift()
    let data = eachSeries.data
    let points = getCandleDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process)
    calPoints.push(points)
    let splitPointList = splitPoints(points, eachSeries)
...
function drawLineDataPoints(series, opts, config, context) {
  ...
  series.forEach((eachSeries, seriesIndex) => {
    ...
    maxRange = ranges.shift()
    let data = eachSeries.data
    let points = getLineDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, lineOption, process)
    calPoints.push(points)
    let splitPointList = splitPoints(points, eachSeries)
    if (eachSeries.lineType == 'dash') {
      let dashLength = eachSeries.dashLength ? eachSeries.dashLength : 8
...
function lonlat2mercator(longitude, latitude) {
  let mercator = Array.from({ length: 2 })
...
function mercator2lonlat(longitude, latitude) {
  let lonlat = Array.from({ length: 2 })
  let x = longitude / 20037508.34 * 180
...
function drawCharts(type, opts, config, context) {
  ...
  let legendHeight = _calLegendData.area.wholeHeight
  let legendWidth = _calLegendData.area.wholeWidth
    ...
    case 'line':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          let _drawLineDataPoints = drawLineDataPoints(series, opts, config, context, process)
          let xAxisPoints = _drawLineDataPoints.xAxisPoints
          let calPoints = _drawLineDataPoints.calPoints
          let eachSpacing = _drawLineDataPoints.eachSpacing
    ...
    case 'scatter':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawScatterDataPoints = drawScatterDataPoints(series, opts, config, context, process)
          let xAxisPoints = _drawScatterDataPoints.xAxisPoints
          let calPoints = _drawScatterDataPoints.calPoints
          let eachSpacing = _drawScatterDataPoints.eachSpacing
          opts.chartData.xAxisPoints = xAxisPoints
          opts.chartData.calPoints = calPoints
          opts.chartData.eachSpacing = eachSpacing
          drawYAxis(series, opts, config, context)
    ...
    case 'bubble':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawBubbleDataPoints = drawBubbleDataPoints(series, opts, config, context, process)
          let xAxisPoints = _drawBubbleDataPoints.xAxisPoints
          let calPoints = _drawBubbleDataPoints.calPoints
          let eachSpacing = _drawBubbleDataPoints.eachSpacing
          opts.chartData.xAxisPoints = xAxisPoints
          opts.chartData.calPoints = calPoints
          opts.chartData.eachSpacing = eachSpacing
          drawYAxis(series, opts, config, context)
    ...
    case 'mix':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawMixDataPoints = drawMixDataPoints(series, opts, config, context, process)
          let xAxisPoints = _drawMixDataPoints.xAxisPoints
          let calPoints = _drawMixDataPoints.calPoints
          let eachSpacing = _drawMixDataPoints.eachSpacing
          opts.chartData.xAxisPoints = xAxisPoints
          opts.chartData.calPoints = calPoints
          opts.chartData.eachSpacing = eachSpacing
          drawYAxis(series, opts, config, context)
    ...
    case 'column':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawColumnDataPoints = drawColumnDataPoints(series, opts, config, context, process)
    ...
    case 'mount':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawMountDataPoints = drawMountDataPoints(series, opts, config, context, process)
    ...
    case 'bar':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawXAxis(categories, opts, config, context)
          let _drawBarDataPoints = drawBarDataPoints(series, opts, config, context, process)
          let yAxisPoints = _drawBarDataPoints.yAxisPoints
          let calPoints = _drawBarDataPoints.calPoints
          let eachSpacing = _drawBarDataPoints.eachSpacing
    ...
    case 'area':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawAreaDataPoints = drawAreaDataPoints(series, opts, config, context, process)
          let xAxisPoints = _drawAreaDataPoints.xAxisPoints
          let calPoints = _drawAreaDataPoints.calPoints
          let eachSpacing = _drawAreaDataPoints.eachSpacing
          opts.chartData.xAxisPoints = xAxisPoints
          opts.chartData.calPoints = calPoints
          opts.chartData.eachSpacing = eachSpacing
          drawYAxis(series, opts, config, context)
    ...
    case 'candle':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context)
          drawXAxis(categories, opts, config, context)
          let _drawCandleDataPoints = drawCandleDataPoints(series, seriesMA, opts, config, context, process)
...
let uCharts = function uCharts(opts) {
...
uCharts.prototype.updateData = function () {
  ...
  switch (scrollPosition) {
    ...
    case 'left':
      ...
      break
    case 'right':
      let _calYAxisData = calYAxisData(this.opts.series, this.opts, this.config, this.context); let yAxisWidth = _calYAxisData.yAxisWidth
      this.config.yAxisWidth = yAxisWidth
      let offsetLeft = 0
      let _getXAxisPoints0 = getXAxisPoints(this.opts.categories, this.opts, this.config); let xAxisPoints = _getXAxisPoints0.xAxisPoints
